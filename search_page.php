...
if(isset($_POST['add_to_wishlist'])){
    ...
    $check_wishlist_numbers = $conn->prepare("SELECT* FROM `wishlist` WHERE id= ? AND size = ?");
    $check_wishlist_numbers->execute([$pid,$p_size]);

    $check_cart_numbers = $conn->prepare("SELECT* FROM `cart` WHERE id= ? AND size = ?");
    $check_cart_numbers->execute([$pid,$p_size]);

    if($check_wishlist_numbers->rowCount() > 0){
        $message[] = 'STOP! Đã nằm trong ds yêu thích!';
    }elseif($check_cart_numbers->rowCount() > 0){
        $message[] = 'STOP! Đã nằm trong giỏ hàng!';
    }else{
        ...
        $message[] = 'thêm thành công vào ds yêu thích!';
    }
}
...
if(isset($_POST['add_to_cart'])){
    ...
    if($check_cart_numbers->rowCount() > 0){
        ...
        if($check_wishlist_numbers->rowCount() > 0){
            $delete_wishlist = $conn->prepare("DELETE FROM `wishlist` WHERE id= ? AND size = ?");
            $delete_wishlist->execute([$pid,$p_size]);
        }
    ...
    <body>
        ...
        <section class="search-form">
             <form action="" method="POST">
                <input type="text" class="box" name="search_box" placeholder="Bạn đang tìm kiếm...">
        ...
        <section class="products">
                ...
                <?php
                        ...
                        echo '<p class="empty">khong tim thay san pham!</p>';
                    }
                    };
                ?>
